...
| `wp[].freq` | the waypoint's `INF`, `COM`, `NAV`, `OTH` properties | Radio frequencies, in the app's own order, already split into a station and a number. Omitted when the waypoint has none. **Only on this document**: the nearby document carries sixty waypoints and their radio would be several kilobytes on a link that moves 244 bytes at a time. |
| `wp[].freq[].k` | | The group the app filed it under: `inf` recorded information, `com` what a pilot calls, `nav` a navaid, `oth` the rest. A client must tolerate a value it does not know. |
| `wp[].freq[].s` | | The station, for example `STUTTGART TOWER`. |
| `wp[].freq[].f` | | The frequency, for example `118.805`, as a string so that trailing zeros survive. **Omitted** when the app knows a station and no number for it, which is not the same as the station not existing. |
...
| `fis` | `GeoMapProvider::airspacesAtPosition()`, category `FIS` | The flight information service sectors the aircraft is inside, in the app's own order. **Omitted** when there is no position, when no sector covers it, or when the pilot's map has no airspace data. Present on this frame rather than in a document of its own because it changes when a sector boundary is crossed, which is a position event: a document polled every few minutes would name the wrong frequency for the minutes in between. The phone recomputes it only when the aircraft has moved 2 km, since the answer costs a polygon test against every airspace it knows. |
| `fis[].s`, `fis[].f` | the airspace's name | Station and frequency, split the same way as `wp[].freq`. The app keeps them inside one string -- `LANGEN – LANGEN INFORMATION 126.950` -- with an en dash between the sector and the station. |
| `fis[].a` | | The sector, for example `ALPINE AREA LANGEN`. It is how a pilot confirms the frequency is the one they think it is. Omitted when the name carries no dash. |
| `fis[].bot`, `fis[].top` | `Airspace::lowerBound()`, `upperBound()` | The vertical band as the app writes it, for example `GND` and `FL 100`. Sectors stack, which is why they travel as a list and why the band matters. |
